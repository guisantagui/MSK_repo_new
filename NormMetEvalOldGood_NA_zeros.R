...
# substitute missing values by zero instead of interpolate them (to see what happens, 
# if this line is run the following one mustn't)
missingMet <- apply(logdataMet$featuredata, 2, function(x){
        a <- x
        a[is.na(a)] <- 0
        return(a)})
...
missingMet <-  MissingValues(missingMet, sampledataMet,metabolitedataMet,
                             feature.cutof=1, sample.cutoff=0.2, method="replace")

IS <- metvar(missingMet$featuredata, 
             log.data[, 1])$metvar[which(metvar(missingMet$featuredata, 
                                                log.data[, 1])$metvar$padj > 0.05), 1]

ISMat <- missingMet$metabolitedata
ISMat$IS <- rep(0, nrow(ISMat))
ISMat$IS[ISMat$names %in% IS] <- 1
missingMet$metabolitedata <- ISMat
