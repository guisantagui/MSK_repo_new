...
load("C:/Users/Guillem/Documents/PhD/comput/wrkng_dirs_clean/solveAmbig/uniqueCandidates.RData")
...
uniqueMetsNames <- uniqueMetsNames[-4]

save(solvedAmbigMets, file = "solvedAmbigMets.RData")

colnames(ccmnNormMets)[match(names(uniqueMetsNames), colnames(ccmnNormMets))] <- uniqueMetsNames
...
pdf("heatmapCCMN_quant.pdf", width = 60, height = 30, pointsize = 25)
heatmap.2(as.matrix(t(ccmnNormMetsNoAmbigQuant)), distfun = function(x) dist(x, method = "euclidean"), 
          density.info = "none", hclust = function(x) hclust(x, method = "ward.D"), dendrogram = "both", 
          col = redgreen(75), breaks = 76, ColSideColors = swarmCols_PC1#[match(rownames(ccmnNormMetsNoAmbigQuant),
          #      rownames(ccmnNormMets))]
          , 
          notecol = NULL, trace = "none", xlab = "Strains", 
          ylab = "Metabolites", 
