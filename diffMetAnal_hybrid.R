setwd("/Users/santamag/Desktop/GUILLEM/wrkng_dirs_clean/diffMetAnal/hybrid")
...
ccmn_norm_mets <- ccmn_norm_mets_hybrid
...
ggsave(filename = "HCA_CCMN_hybrid.pdf", plot = last_plot(), device = "pdf", 
       path = NULL,
...
compKEGGIDs <- dictionary$`KEGG IDs`[match(colnames(ccmn_norm_mets), dictionary$Consensus)]
...
tiff("heatmapCCMN_quant_hybrid.tiff", width = 5000, height = 5000, units = "px", pointsize = 50)
...
diffMets_hybrid <- diffMets
save(diffMets_hybrid, file = "diffMets_hybrid.RData")
write.csv(diffMets, file = "diffMets_allClusts_hybrid.csv")
