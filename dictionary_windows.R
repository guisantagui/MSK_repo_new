setwd("C:/Users/Guillem/Documents/PhD/comput/wrkng_dirs_clean/dictionary")
...
# Windows changed order of compounds when creating metabolitepeaks object, putting 2'5-Dioxopentanoate 2 before all the compounds starting 
# with 2 because of the apostrophe. This is messing all the dictionary. Let's reorder it
presOld <- rbind(presOld[1:2, ],
                 presOld[4:6, ],
                 presOld[3, ],
                 presOld[7:nrow(presOld), ])
...
defNames <- keggList("compound")[match(gsub("*", "", 
                                            dictionary$`KEGG IDs`, 
                                            fixed = T), 
                                       gsub("cpd:", 
                                            "", 
...
defNames <- sapply(defNames, function(x) unlist(strsplit(x, ";"))[1])

defNames[grep("*", dictionary$`KEGG IDs`, fixed = T)] <- paste(defNames[grep("*", 
                                                                             dictionary$`KEGG IDs`, 
                                                                             fixed = T)],
                                                               "(repeated)")
dictionary$definitiveNames <- defNames

dictionary$definitiveNames[12] <- "AICAR"

dictionary$definitiveNames[49] <- "Hexose phosphate"
dictionary$definitiveNames[66] <- "Succinate semialdehyde (redundant)"
dictionary$definitiveNames[70] <- "Hexose diphosphate"
dictionary$definitiveNames[79] <- "2-Methylmaleate (redundant)"
dictionary$definitiveNames[86] <- "O-Acetyl-L-homoserine (redundant)"
